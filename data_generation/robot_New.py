...
class Robot(object):
    ...
    def __init__(self, is_sim, obj_mesh_dir, num_obj, shapes, workspace_limits,
        ...
        for ind, shape in enumerate(shapes):
            if shape in self.shapes_dict:
                self.shapes_dict[shape].append(ind)
            else:
                self.shapes_dict[shape] = [ind]
            self.shapes_dict_r[ind] = shape.lower()
        self.added_shapes = dict.fromkeys(shapes, 0)
        self.shapes = shapes
        self.is_sim = is_sim
    ...
    def add_objects(self):
        ...
        for object_idx in range(self.num_obj):
            ...
            #hmd
            object_color_r = self.obj_mesh_color[self.shapes_dict[curr_shape_name][self.added_shapes[curr_shape_name]]][0]
            object_color_g = self.obj_mesh_color[self.shapes_dict[curr_shape_name][self.added_shapes[curr_shape_name]]][1]
            object_color_b = self.obj_mesh_color[self.shapes_dict[curr_shape_name][self.added_shapes[curr_shape_name]]][2]
            #
            ...
            object_color = [object_color_r, object_color_g, object_color_b]
            object_static = 0
            print(self.sim_client, vrep.sim_scripttype_childscript, [object_static], object_position + object_orientation + object_color, [curr_mesh_file, curr_shape_name], vrep.simx_opmode_blocking)
            ...
            if ret_resp == 8:
                ...
                # hmd
                self.added_shapes[curr_shape_name] += 1
    ...
    def get_shape_color_json(self):
        shape_color = {}
        for key, value in self.shapes_dict_r.items():
            object_color = (self.obj_mesh_color[key][0],
                            self.obj_mesh_color[key][1],
                            self.obj_mesh_color[key][2])
            shape_color[str(tuple(object_color))] = value
        return json.dumps(shape_color)
